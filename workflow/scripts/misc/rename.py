...
"""
Given input files and hot/cold storage mounting points, this script:
* copies files from cold storage to working directory.
* links files from hot storage to working directory to ensure naming interface.

While performing these steps, if multiple files are provided and point to
a unique output file, then this script handles on-the-fly concatenation.

Due to iRODS storage file system being present in Gustave Roussy,
this script also handles iRODS interface through bash interface.
"""
